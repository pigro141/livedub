...
下にあるのは音のない四つのクリップです。README は音を鳴らせません。GitHub は GIF
...
GTA VI のトレーラーの同じ十三秒を、四回吹き替えたものです。公式字幕は GTA V の
書き方どおりに**映像へ焼き込んで**あります — 文字の大きさと位置は、ゲームの
実際の一コマから測ったもので、当て推量ではありません — そしてこのプログラムは
それを画素から読み直しました。ここでは何も翻訳していません。それぞれの回は、
すでにその言語で書かれている字幕を読み、**その言語の** Kokoro の声で言います。
黒い帯に名前が出ていれば — `[nicola-2_5]`、`[es_santa]` — 声紋が**誰が**
話しているかを確定できた、という意味です。この場面を電話の会話にしたのは
そのためで、声紋には続けて話す人が必要です。せりふが一秒しか続かないところ
では、正直な答えは `[neutra]`、*まだ誰が話しているか分からない* のままです。

[![四つの言語](../../assets/vetrina/trailer-multilingua.gif)](https://pigro141.github.io/livedub/?lang=ja#watch)
...
| **せりふを 52 言語で言う** | piper で 49、supertonic で 31、kokoro で 8。言語を選べばエンジンがついてくる |
...
— 42 言語、41 のカタログはどれも完成しており、282 個中 282 個です。アラビア語・
...
| Python | 3.11 | 3.11 |
| **得られるもの** | **CPU の Piper。** 字幕から声まで 665 ミリ秒、音切れなし、話速の詰めもなし。読み取りは PP-OCR で、話せる 52 言語のうち 49 はすでにここにあります。 | **CUDA の Kokoro**。発音がよく、8 言語 54 の声を持ちます。1290 ミリ秒。 |
...
ウィンドウを組み立てます。その 13 個の検査がすべて通らなければ、何も公開されません。
検査を通ったパッケージは[リリースのページ](https://github.com/pigro141/livedub/releases/latest)にあります —
約 420 MB の zip と、その隣に SHA256。GitHub のアカウントは要らず、期限もありません。

**限界は二つ、隠さずに書きます。** **Smart App Control** が有効だと — Windows 11 を
まっさらに入れた状態では既定で有効です — 実行ファイルは**起動しません**。ビルドはどれも
新しいファイルで、新しいファイルは仕組み上まだ評判を持たないからです。これを動かすのは
署名であって、別の検証ではありません。そしてビルドマシンにはサウンドカードもグラフィック
カードもなく、ゲームも走っていません。画面キャプチャ、音声のループバック、ミキシング、
GPU での合成は**未検証のまま**です。しかもそこでは Smart App Control も切られているので、
「ランナーでは起動する」は「入れたばかりの Windows 11 で起動する」を意味しません。
...
**インストールは意図的に軽くなり、オフライン翻訳が組み込まれるようになりました。** 以前は **3100 MB** の費用がかかり、省略されていました - そのほとんどすべてが `torch` であり、翻訳では決して使用されませんでした。この 3 ギガバイトは `stanza` で取得され、Argos は *文を分割*するだけで済みます。MiniSBD では、その作業に **178 KB** のコストがかかり、エンジン全体の重さは実測で **66 MB** になります。したがって、上記の 2 つのコマンドとともに到着し、**実行可能ファイル内**を移動します。つまり、ソースとパッケージから同じ文が 1 文字ずつ出力されます。言語ペアはまだモデル (98 MB) であり、[スタート] を押すとベンチが自動的にダウンロードします。
...
| **声に出して言える**言語 | **52** — ただしどのエンジンでもというわけではありません: piper で 49、supertonic で 31、kokoro で 8 | あなたが言語を選ぶと、エンジンがそれに従います |

> **三つの一覧、三つの問い。** 画面表示は 42 言語、翻訳は 133 言語に届き、口は 52
> 言語を話します。この最後の数字は一つの数字ではありません: **三つのエンジンは
> それぞれ別のカタログを持ち**、言語を選ぶことは実のところエンジンを選ぶことなの
> です。52 になる変更の前、口が話せたのは**二つ**でした — それはエンジンの限界では
...
|---|---|---|---|---|
| **piper** *(既定)* | **49** | 公式の索引に 175 のモデル | CPU | 声ごとに一つのモデル、それぞれ個別にダウンロード（28〜114 MB） |
...
<details>
<summary><b>52 言語すべて、エンジンごと</b> — ✓ は、そのエンジンがその言語に自前の声を少なくとも一つ持っているという意味です。</summary>
...
列を縦に読めば、そのエンジンのカタログになります。一つのエンジンだけが話す言語: `piper` 20 · `supertonic` 2 (Croatian, Lithuanian) · `kokoro` 0。三つとも話す言語: 6 — English, French, Hindi, Italian, Portuguese, Spanish。
...
> **主張していないこと。発音がよい、とは言っていません。** 52 言語を聴いた人は誰も
...
| **piper** | **49 中 0** | なし。このマシンでは Smart App Control が `espeakbridge.pyd` を止めており、piper の声は一つを除いてすべて espeak で音素化します。唯一あった測定 — ヘブライ語の毎秒 9.14 文字 — は、その言語ごと取り下げました。固定版の `piper-tts` はヘブライ語を音素化できないので、49 にも入っていません |
...
> **検査が取り下げさせた主張が二つ、しかも二つめは私たち自身のものでした。**
> piper の索引は **51** 言語を挙げ、このプログラムは **49** を提供します。入って
> いる固定版の `piper-tts`（1.3.0）が知っている音素化器はちょうど二つ —
> `PhonemeType` は `["espeak", "text"]` — で、二つの声はそれ以外を要求します。
> 日本語は `japanese` を、ヘブライ語は `hebrew` を求めます。どちらのモデルも平然と
> 落ちてきて、読み込みで失敗します。
>
> 語るに値するのはヘブライ語のほうです。このページはかつて、`piper-tts` が四つの
> 音素化器を知っていて、ヘブライ語が*動作し、毎秒 9.14 文字と測定された*と述べて
> いました。固定版の列挙型はそう言っておらず、固定した依存関係が再現できない測定は
> 測定ではありません — それは別のインストールの記憶です。この数はいまやその列挙型
> から導かれるので、二度とずれません。日本語は kokoro か supertonic でそれでも
> 話せます。**ヘブライ語は、ここではどれにも話せません** — このプログラムのどの
> エンジンにもヘブライ語の声はありません。
...
いる言語であるイタリア語を足したものです。41 はどれも**完成しており、282 個中 282
...
| | |
|---|---|
| **52 言語を聴いた人は誰もいません** | 確かめてあるのは、声が存在すること、その声がその言語のものであること、そして測れたところでは話す速さがもっともらしいことです。発音は確かめていません。そしてイタリア語が、このプログラムが作られ、聴かれてきた言語です。 |
...
pytest はありません。テスト一式は実行できるモジュールで、86 グループに **2546 個の
検査**があります。
