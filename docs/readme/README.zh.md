...
| **用 52 种语言说出台词** | piper 49 种，supertonic 31 种，kokoro 8 种；你选语言，引擎跟着走 |
...
词表每一份都是完整的：282 条中的 282 条。阿拉伯语、希伯来语、波斯语和乌尔都语还会
把整个窗口翻转过来。
...
| Python | 3.11 | 3.11 |
| **你得到什么** | **CPU 上的 Piper。** 从字幕到嗓音 665 毫秒，没有断音，也不用把话说快。读取用的是 PP-OCR，而 52 种可说语言里已经有 49 种在这里了。 | **CUDA 上的 Kokoro**：咬字更清楚，带着它 8 种语言里的 54 把嗓音。1290 毫秒。 |
...
然后*运行*它：在包内读取一条画出来的字幕、合成一句台词、搭出窗口；这 13 项检查只要有一项
不过，就什么都不发布。通过检查的包在[发布页](https://github.com/pigro141/livedub/releases/latest)上 ——
一个约 420 MB 的 zip，旁边放着它的 SHA256，不需要 GitHub 账号，也没有有效期。
...
| 它能**开口说**哪些语言 | **52** —— 但不是每个引擎都行：piper 49 种，supertonic 31 种，kokoro 8 种 | 你选语言，引擎跟着走 |

> **三份清单，三个问题。** 界面会说 42 种语言，翻译能到 133 种，而这张嘴能说 52
> 种。最后这个数字并不是一个数字：**三个引擎各有各的目录**，选一种语言其实就是在选
> 一个引擎。在变成 52 之前，这张嘴只会说**两种** —— 那从来不是引擎的极限，而是代码
...
|---|---|---|---|---|
| **piper** *(默认)* | **49** | 官方索引里有 175 个模型 | CPU | 一把嗓音一个模型，各下各的（28–114 MB） |
...
<details>
<summary><b>全部 52 种语言，逐个引擎</b> — ✓ 表示该引擎在这种语言里至少有一把自己的嗓音。</summary>
...
竖着读一列，就是那个引擎的目录。只有一个引擎会说的：`piper` 20 · `supertonic` 2 (Croatian, Lithuanian) · `kokoro` 0。三个引擎都会说的：6 — English, French, Hindi, Italian, Portuguese, Spanish。
...
> **没有声称的：发音好不好。** 没有人听过 52 种语言，说别的话，就是一句没有任何
> 测量支撑的承诺。
...
| **piper** | **49 中 0** | 没有。这台机器上 Smart App Control 拦住了 `espeakbridge.pyd`，而 piper 的嗓音除一个之外都通过 espeak 做音素转换。原本唯一的那次测量 —— 希伯来语每秒 9.14 个字符 —— 已经和这门语言一起撤回：锁定的 `piper-tts` 不会音素转换希伯来语，所以它也不在这 49 种里 |
...
> **检查拿走了两条声称，而第二条是我们自己的。** piper 的索引列出 **51** 种语言，
> 而这个程序提供 **49** 种。已安装并锁定的 `piper-tts`（1.3.0）只认识两个音素转换器
> —— `PhonemeType` 是 `["espeak", "text"]` —— 而有两把嗓音要的是别的：日语要
> `japanese`，希伯来语要 `hebrew`。两个模型都下载得很顺利，一加载就报错。
>
> 值得一讲的是希伯来语。这个页面曾经声称 `piper-tts` 认识四个音素转换器，而希伯来语
> *可用，测得每秒 9.14 个字符*。锁定版本的枚举并不这么说，而一个被锁定的依赖无法
> 重现的测量不是测量 —— 那是另一次安装留下的记忆。这个数字现在由那个枚举推导出来，
> 所以不会再走样。日语照样能说，由 kokoro 或 supertonic 来说；**而希伯来语在这里
> 谁也说不了** —— 这个程序里没有一个引擎有希伯来语的嗓音。
...
41 份全都**完整，282 条中的 282 条**，没有一份只翻了一半；其中四种从右往左读，会把
...
| | |
|---|---|
| **没有人听过这 52 种语言** | 已验证的是：一把嗓音存在、它属于那种语言，以及在能测的地方它的语速是合理的。发音没有验证，而意大利语是这个程序被造出来、也被反复听过的那种语言。 |
...
没有 pytest：测试套件是一个可以直接运行的模块，86 组里共 **2520 项检查**。
