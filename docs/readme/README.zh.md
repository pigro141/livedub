...
**目前尚未发布可执行文件。** livedub 按上文所述从源码安装。只有在构建机器真正启动并验证之后，可执行文件才会出现在这里。
...
**安装是刻意做轻的，而且刻意把一样东西留在外面。** 离线翻译**不会安装**：它要
**3100 MB**，其中几乎全是 `torch` —— 翻译从来不用它，可是没有它，翻译的断句模块连
导入都做不到。为一项**默认关闭**的功能，让每一个装这个程序的人都付这笔账，是选择
的反面。它在**你需要的时候**才来：向导里的试验台看缺什么，在你决定**之前先声明它
有多重**，然后把要粘贴的那一行交给你。之所以是交给你而不是替你执行，是因为那些是
*软件包*，而在那里天真地 `pip install` 恰恰会把 CPU 版的轮子重新拉回来。如果它没有
到位，那是一次**被声明的放弃**，不是一次无声的退让。至于语言对本身，那是一个 98 MB
的模型，那个试验台会自己下载。
