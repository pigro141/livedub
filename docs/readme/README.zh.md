...
词表每一份都是完整的：281 条中的 281 条。阿拉伯语、希伯来语、波斯语和乌尔都语还会
把整个窗口翻转过来。
...
| 它能**开口说**哪些语言 | **53** —— 但不是每个引擎都行：piper 49 种，supertonic 31 种，kokoro 8 种 | 你选语言，引擎跟着走 |
...
41 份全都**完整，281 条中的 281 条**，没有一份只翻了一半；其中四种从右往左读，会把
...
没有 pytest：测试套件是一个可以直接运行的模块，85 组里共 **2460 项检查**。
