...
class SubtitleReader(Stage):
    ...
    def __init__(
        ...
        **kw,
        ...
        # Quante scritte si sono trovate, e **quante se ne sono buttate** perche'
    ...
    def process(self, frame: np.ndarray | None) -> TrackerOutput:
        ...
        t0 = time.perf_counter()
        bande = self._bande(roi, frame)
        self._t_classify.add((time.perf_counter() - t0) * 1000.0)

        # Ogni riga si porta dietro **a quale scritta appartiene**: con un'area
        # normale la scritta e' una sola e l'indice vale sempre zero, cioe' il
        # comportamento di prima. Con un'area `schermo` sono N, e mescolarle
        # vorrebbe dire una battuta sola che comincia in un angolo dello schermo
        # e finisce nell'altro.
        lines: list[tuple[int, OcrLine]] = []
        for gruppo, band in bande:
        ...
        # Una battuta per scritta, non una per fotogramma. `dict.fromkeys` tiene
        # l'ordine di lettura: conta per chi legge un log, e non costa niente.
        candidates = []
